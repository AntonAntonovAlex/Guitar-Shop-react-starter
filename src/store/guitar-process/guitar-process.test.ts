...
describe('Reducer: guitarProcess', () => {
  it('should have resetCountReviews', () => {
    expect(guitarProcess.reducer({activPage: 1, countReviews: 15, idCardForCart: 0, guitarCart: new Map()}, resetCountReviews()))
      .toEqual({activPage: 1, countReviews: STEP_COUNT_REVIEWS, idCardForCart: 0});
  });
  ...
  it('should increment current count by a given value', () => {
    const state = {activPage: 1, countReviews: STEP_COUNT_REVIEWS, idCardForCart: 0, guitarCart: new Map()};
    expect(guitarProcess.reducer(state, incrementCountReviews()))
      .toEqual({activPage: 1, countReviews: 6, idCardForCart: 0});
  });

  it('should change the genre to the given value', () => {
    const state = {activPage: 1, countReviews: STEP_COUNT_REVIEWS, idCardForCart: 0, guitarCart: new Map()};
    expect(guitarProcess.reducer(state, changeActivPage(8)))
      .toEqual({activPage: 8, countReviews: STEP_COUNT_REVIEWS, idCardForCart: 0});
  });
